...
def parse_json(json_string):
    ...
    for item in items:
        ...
        date_dict = {}
        text_dict = {}
        text_dict["headline"] = '<a href="http://chroniclingamerica.loc.gov{}.pdf">{}</a>'.format(item["id"], item["title"])
        print(text_dict["headline"], sys.stderr)
        text_dict["text"] = item["ocr_eng"]
        date_dict["year"] = item["date"][0:4]
        date_dict["month"] = item["date"][4:6]
        date_dict["day"] = item["date"][6:8]
        media_dict["url"] = 'http://chroniclingamerica.loc.gov{}.thumbnail.jpg'.format(item["id"])
